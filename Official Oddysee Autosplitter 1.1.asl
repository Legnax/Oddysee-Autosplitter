...
// ############ PAUL'S BLACK MAGIC ENDS HERE (STARTED AT LINE 78) ##################
	...
	if (File.Exists(@"C:\Autosplit Backup Files\n")) { // We conveniently recover the Oddysee current split 
		...
		vars.n = 8;		// CHANGE LATER!! - A counting variable that tracks progress (only resets if game loads on proper way).
	}
...
{	
	...
	if (vars.StartgnFrame > 10){ // This should ONLY happen when the autosplitter binds to the game when you start a new game, not when you are restarting the game. 
		vars.n = 8; // First split if you press on start game
		vars.PauseStartTime = -1;
		vars.MillisecondsPaused = 0;
		vars.PreviousTime = 0;
